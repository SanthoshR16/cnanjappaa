...
try {
  ...
  if (fs.existsSync(serviceAccountPath)) {
    ...
    admin.initializeApp({
      projectId: process.env.FIREBASE_PROJECT_ID || 'c-nanjappa-timber-traders'
    });
    console.log("SUCCESS: Firebase Admin initialized successfully using Project ID (Fallback).");
  }
} catch (e) {
  console.error("FAILURE initializing Firebase Admin: " + e.message);
}
...
app.post('/api/upload', async (req, res) => {
  try {
    ...
    console.warn("Cloudinary upload failed, falling back to local base64 image url:", err.message);
    // Return the base64 string directly so local database storing and rendering still work
    res.json({ secure_url: req.body.file });
  }
});
