...
export const API_URL = (import.meta.env.VITE_API_URL as string) || (window.location.hostname === 'localhost' ? 'http://localhost:3000/api' : '/api');

const fetchWithAuth = async (url: string, options: RequestInit = {}) => {
  const isE2E = (typeof window !== 'undefined') && (window.location.search.includes('e2e=true') || localStorage.getItem('e2e') === 'true');
  const token = isE2E ? 'mock-token-qa-tester' : (auth.currentUser ? await auth.currentUser.getIdToken() : null);
...
export const AppContextProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    if (isE2E) {
      const savedUser = localStorage.getItem('e2e_user');
      if (savedUser) {
        setManagerProfile(JSON.parse(savedUser));
  ...
  const logout = async () => {
    if (isE2E) {
      ...
      setIsAuthenticated(false);
      setManagerProfile(null);
      addToast('Successfully logged out (E2E Mock)', 'success');
      return;
    }
    try {
      await signOut(auth);
  ...
  const loginWithGoogle = async () => {
    if (isE2E) {
      const mockUser = {
        ...
        displayName: 'Google QA User',
        emailVerified: true,
      } as any;
      localStorage.setItem('e2e_user', JSON.stringify(mockUser));
      setManagerProfile(mockUser);
      setIsAuthenticated(true);
      addToast('Logged in with Google successfully (E2E Mock)!', 'success');
      fetchData();
      return;
    }
  ...
  const login = async (email?: string, password?: string) => {
    if (isE2E) {
      if (email && password) {
        ...
        setManagerProfile(mockUser);
        setIsAuthenticated(true);
        addToast('Logged in successfully (E2E Mock)!', 'success');
        fetchData();
        return;
      } else {
    ...
    if (isE2E) {
      if (email && password) {
        const mockUser = {
          uid: 'qa-tester-id',
          email: email,
          displayName: fullName || 'QA Automation User',
          emailVerified: true,
        } as any;
        localStorage.setItem('e2e_user', JSON.stringify(mockUser));
        setManagerProfile(mockUser);
        setIsAuthenticated(true);
        addToast('Account created successfully (E2E Mock)!', 'success');
        fetchData();
        return;
      } else {
  ...
  const resetPassword = async (email: string) => {
    if (isE2E) {
      if (email) {
        addToast('Password reset link sent successfully (E2E Mock).', 'success');
        return;
      } else {
