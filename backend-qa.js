...
try {
  serviceAccount = JSON.parse(fs.readFileSync(path.join(__dirname, 'server/firebase-service-account.json'), 'utf8'));
} catch (e) {
  console.warn("Firebase service account JSON not found. Running tests in offline mock mode.");
}
...
async function getAuthToken() {
  if (!serviceAccount) {
    console.log("No Firebase service account found. Using mock QA token.");
    return "mock-token-qa-tester";
  }
  ...
  try {
    ...
    console.warn("Failed to exchange custom token with Google API, falling back to mock QA token:", err.message);
    return "mock-token-qa-tester";
  }
}
